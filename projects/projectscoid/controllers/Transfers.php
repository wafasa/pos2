...
class Transfers extends MY_Controller
{
    ...
    function getTransfers()
    {
        ...
        $excel_link = anchor('admin/transfers/excel/$1', '<i class="fa fa-file-excel-o"></i> Download as Excel');
    ...
    {
        $this->sma->checkPermissions();
        // $this->sma->print_arrays($_POST);die;
        ...
        if ($this->form_validation->run()) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_unit_quantity = str_replace(",", "", substr($_POST['quantity'][$r], 0, strpos($_POST['quantity'][$r], '.')));
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    $row = 1;
                    foreach ($_POST['val'] as $id) {
                         $this->excel->getActiveSheet()->SetCellValue('A' . $row, lang('date'));
                    $this->excel->getActiveSheet()->SetCellValue('B' . $row, lang('reference_no'));
                    $this->excel->getActiveSheet()->SetCellValue('C' . $row, lang('from_warehouse'));
                    $this->excel->getActiveSheet()->SetCellValue('D' . $row, lang('to_warehouse'));
                    $this->excel->getActiveSheet()->SetCellValue('E' . $row, lang('grand_total'));
                    $this->excel->getActiveSheet()->SetCellValue('F' . $row, lang('status'));
                        ...
                        $dts = $this->transfers_model->getAllTransferItems($id, $tansfer->status);
                        
                          $this->excel->getActiveSheet()->SetCellValue('A' . $row, lang('description'));
                            $this->excel->getActiveSheet()->SetCellValue('B' . $row, lang('quantity'));
                           
                            $this->excel->getActiveSheet()->SetCellValue('C' . $row, lang('expiry'));
                        ...
                        foreach($dts as $t)
                        {
                            ...
                            $this->excel->getActiveSheet()->SetCellValue('A' . $row, $t->product_code . " - " . $t->product_name);
                            $this->excel->getActiveSheet()->SetCellValue('B' . $row, ($t->unit_quantity).' '.$t->product_unit_code);
                             
                            if($row->expiry && $row->expiry != '0000-00-00') { $this->excel->getActiveSheet()->SetCellValue('C' . $row, date('d-M-Y', strtotime($row->expiry))); 
                                } else { 
                                $this->excel->getActiveSheet()->SetCellValue('C' . $row, "01-01-2020");
                                }
                            $this->excel->getActiveSheet()
                                ->getStyle('D'.$row)
                                ->getNumberFormat()
                            ...
                            $this->excel->getActiveSheet()->SetCellValue('D' . $row, ($t->subtotal));

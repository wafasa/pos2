...
$(document).ready(function() {
    ...
    $.ajax({
        type: 'GET',
        url: site.base_url +'code/getCode',
...
$('#reset').on("click", function() {
    var temp = { data : localStorage.getItem('positems'),
                customer : $(".select2-chosen").html() };
                // console.log(localStorage.getItem('positems'));
                // return;
    $.ajax({
        type: 'POST',
        url: site.base_url + 'pos/add_cart',
        data: temp,
...
function loadItems() {
    if (localStorage.getItem('positems')) {
        ...
        $.each(sortedItems, function () {
			...
			totalTrue = totalTrue + item.row.price;
        ...
        if(totalTrue > 0)
				$('#total').text(formatMoney(total)).append("&nbsp;<del>(" + formatMoney(totalTrue) + ")</del>");
			else $('#total').text(formatMoney(total));

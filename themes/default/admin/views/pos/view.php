...
<div class="modal-dialog no-modal-header" role="document"><div class="modal-content"><div class="modal-body">
    ...
    <head>
        ...
        <style type="text/css" media="all">
            body { color: #000; }
            #wrapper { max-width: 280px; margin: 0px -50px 0px 0px !important; padding-top: 0px; }
            .btn { border-radius: 0; margin-bottom: 5px; }
            .bootbox .modal-footer { border-top: 0; text-align: center; }
            h3 { margin: 5px 0; }
            p { font-size: 11px!important; font-weight: 600 !important;}
            td { font-size: 11px!important; font-weight: 600 !important;}
            th { font-size: 15px!important; font-weight: 700 !important;}
            hr { width: 480px; border: 5px !important; }
            .order_barcodes img { float: none !important; margin-top: 5px; }
            @media print {
                .no-print { display: none; }
                #wrapper { max-width: 530px; width: 100%; min-width: 250px; margin: 0 auto; }
    ...
    <div id="wrapper">
        <div id="receiptData">
            ...
            <div id="receipt-data">
                ...
                <?= $customer->award_points != 0 && $Settings->each_spent > 0 ? '<p class="text-center" style="font-size: 11px !important;">'.lang('this_sale').': '.floor(($inv->grand_total/$Settings->each_spent)*$Settings->ca_point)
                .'</p>' : ''; ?>
                <?= $inv->note ? '<p class="text-center" style="font-size: 11px !important;">' . $this->sma->decode_html($inv->note) . '</p>' : ''; ?>
                <?= $inv->staff_note ? '<p class="no-print" style="font-size: 11px !important;"><strong>' . lang('staff_note') . ':</strong> ' . $this->sma->decode_html($inv->staff_note) . '</p>' : ''; ?>
                <?= $biller->invoice_footer ? '<p class="text-center" style="font-size: 11px !important;">'.$this->sma->decode_html($biller->invoice_footer).'</p>' : ''; ?>
            </div>
<div><p class="text-center" style="font-size: 13px !important;">FB: naturerepublicIndonesia<br>IG: @naturerepublic.id<br>LINE: @naturerepublic.id</p></div>
...
<?php 
    ...
    <script type="text/javascript">
        ...
        $.ajax({
            type: 'GET',
            url: 'http://naturerepublic.id/pos/admin/code/getCode',

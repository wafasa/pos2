...
class system_settings extends MY_Controller
{
    ...
    function banner()
    {
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('banner')));
        $meta = array('page_title' => lang('banner'), 'bc' => $bc);
        $this->page_construct('settings/banner', $meta, $this->data);
    }
    ...
    function add_banner()
    {
        ...
        if ($this->form_validation->run() == true) {
            $banner_name = $_FILES['userfile']['name'];
            $banner_type = $_FILES['userfile']['type'];
            $banner_size = $_FILES['userfile']['size'];
            $banner_tmp = $_FILES['userfile']['tmp_name'];

            if ($_FILES['userfile']['size'] > 0) {
                $this->load->library('upload');
                $config['upload_path'] = $this->upload_path;
                $config['allowed_types'] = 'jpg|jpeg|png|mp4|gif';
                $config['overwrite'] = FALSE;
                ...
                $src = $this->upload->file_name;
                $data['banner_src'] = $src;
                $data['banner_type'] = $this->upload->file_type;
            }

        } elseif ($this->input->post('add_banner')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/banner");
        }

        if ($this->form_validation->run() == true && $this->settings_model->addBanner($data)) {
            $this->session->set_flashdata('message', lang("banner_added"));
            admin_redirect("system_settings/banner");
        } else {

            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->load->view($this->theme . 'settings/add_banner', $this->data);
        }
    }
    ...
    function getBanners() {
        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix(('banner'))}.banner_id as id, {$this->db->dbprefix(('banner'))}.banner_src, {$this->db->dbprefix(('banner'))}.banner_type, {$this->db->dbprefix(('banner'))}.banner_date, {$this->db->dbprefix(('banner'))}.status", FALSE)
            ->from("banner")
            ->add_column("Actions", "<div class=\"text-center\">
                                        <a href='#' class='tip po' style='margin-right:20px' title='<b>" . "Set As Active" . 
                                            "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p>
                                            <a class='btn btn-danger po-delete' href='" . admin_url('system_settings/active_banner/$1') . "'>" . 
                                                lang('i_m_sure') . 
                                            "</a> 
                                        ...
                                        <a href='#' class='tip po' title='<b>" . lang("delete_banner") . 
                                            "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p>
                                            <a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_banner/$1') . "'>" . 
                                                lang('i_m_sure') . 
                                            "</a> 
                                            <button class='btn po-close'>" . lang('no') . 
                                            "</button>\"  rel='popover'>
    ...
    function delete_banner($id = NULL)
    {
        ...
        if ($this->settings_model->deleteBanner($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("banner_deleted")));
        }
    }

    function active_banner($id = NULL)
    {
        $this->db->update("sma_banner", array("status" => "non-active"));
        $this->db->update("sma_banner", array("status" => "active"), array("banner_id" => $id));
        $this->sma->send_json(array('error' => 0, 'msg' => "Banner Update"));
    }
}

            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
						...
						$no = 1;
						foreach($detail as $j){
							$kode = $j->product_code.' - '.$j->product_name . ($j->variant ? ' (' . $j->variant . ')' : '');
							$kode .= $j->details ? '<br>' . $j->details : '';
							$kode .= $j->serial_no ? '<br>' . $j->serial_no : '';
							
							$qty = $this->sma->formatQuantity($j->quantity).' '.$j->product_unit_code; 
							$price = $this->sma->formatMoney($j->real_unit_price); 
							$row++;
							...
							$this->excel->getActiveSheet()->SetCellValue('E' . $row, $price);

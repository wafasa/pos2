...
class Sales extends MY_Controller
{
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment)) {
            ...
            $ref = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('so');
            
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Menambah sales dengan nomor referensi $ref', 'sma_sales', '$ip', '$id')");
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products)) {
            ...
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Memperbarui sales dengan nomor referensi $ref', 'sma_sales', '$ip', '$id')");
    ...
    public function return_sale($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->addSale($data, $products, $payment, $si_return)) {
            // RETURN STOK
            foreach ($si_return as $key => $value) {
                $get_qty = $this->db->select("*")->from("warehouses_products")->where(array("product_id" => $value["product_id"], "warehouse_id" => $value["warehouse_id"]))->get()->row();
                $this->db->update("warehouses_products", array("quantity" => ($get_qty->quantity + $value["quantity"])), array("product_id" => $value["product_id"], "warehouse_id" => $value["warehouse_id"]));

                $get_product_return = $this->db->select("*")->from("sma_products")->where(array("id" => $value["product_id"]))->get()->row();
                $this->db->update("sma_products", array("quantity" => ($get_product_return->quantity + $value["quantity"])));
    ...
    {
        $query = $this->db->query("SELECT reference_no FROM sma_sales WHERE id = '$id'")->result();
        $ref = $query[0]->reference_no;
        ...
        if ($this->sales_model->deleteSale($id)) {
            ...
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Menghapus sales dengan nomor referensi $ref', 'sma_sales', '$ip', '$id')");
    ...
    public function delete_return($id = null)
    {
        ...
        if ($this->sales_model->deleteReturn($id)) {
            if ($this->input->is_ajax_request()) {
                ...
                $ip = $_SERVER['REMOTE_ADDR'];
                $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Menghapus return dengan nomor referensi $id', 'sma_return', '$ip', '$iduser')");
            ...
            if (!empty($_POST['val'])) {
                if ($this->input->post('form_action') == 'delete') {
                    ...
                    foreach ($_POST['val'] as $id) {
						...
						$this->excel->getActiveSheet()->SetCellValue('C' . $row, lang("description"));
						$this->excel->getActiveSheet()->SetCellValue('E' . $row, lang("quantity"));
						$this->excel->getActiveSheet()->SetCellValue('F' . $row, '');
						$this->excel->getActiveSheet()->SetCellValue('G' . $row, lang("tax"));
						$this->excel->getActiveSheet()->SetCellValue('H' . $row, lang("tax"));
						$this->excel->getActiveSheet()->SetCellValue('I' . $row, lang("unit_price"));
						$this->excel->getActiveSheet()->SetCellValue('J' . $row, lang("subtotal"));
						$this->excel->getActiveSheet()->SetCellValue('K' . $row, lang("Sale id"));
						...
						foreach($detail as $j){
							...
							$kode 			= $j->product_code;
							$kode2 			= $j->product_name;
							$qty 			= $j->quantity;
							$satuan 		= $j->product_unit_code; 
							$diskon 		= $j->item_tax != 0 && $j->tax_code ? $j->tax_code:'';
							$nilai_diskon 	= $j->item_tax;
							$price 			= $j->real_unit_price; 
							$sub_total 		= $j->subtotal;
							$id_sale		= $j->sale_id;
							
							$row++;
							$this->excel->getActiveSheet()->SetCellValue('B' . $row, $no);
							$this->excel->getActiveSheet()->SetCellValue('C' . $row, $kode);
							$this->excel->getActiveSheet()->SetCellValue('D' . $row, $kode2);
							$this->excel->getActiveSheet()->SetCellValue('E' . $row, $qty);
							$this->excel->getActiveSheet()->SetCellValue('F' . $row, $satuan);
							$this->excel->getActiveSheet()->SetCellValue('G' . $row, $nilai_diskon);
							$this->excel->getActiveSheet()->SetCellValue('H' . $row, $diskon);
							$this->excel->getActiveSheet()->SetCellValue('I' . $row, $price);
							$this->excel->getActiveSheet()->SetCellValue('J' . $row, $sub_total);
							$this->excel->getActiveSheet()->SetCellValue('K' . $row, $id_sale);
...
if (file_exists($target)) {
    ...
    public function delete_payment($id = null)
    {
        ...
        if ($this->sales_model->deletePayment($id)) {
            ...
            $iduser = $this->session->userdata('user_id');
                $ip = $_SERVER['REMOTE_ADDR'];
                $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Menghapus payment dengan nomor referensi $id', 'sma_payment', '$ip', '$iduser')");
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            ...
            foreach ($rows as $row) {
                ...
                // print_r($row->price );
                // var_dump( $customer_group->percent / 100);
                $save_price = $row->price = $row->price + (($row->price * $customer_group->percent) / 100);

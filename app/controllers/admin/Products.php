...
class Products extends MY_Controller
{
	...
	function discount($warehouse_id = NULL)
    {
        $this->sma->checkPermissions();
		...
		$sql = "select  
			id, 
			product_id,
			coalesce((select name from sma_products where id = product_id), 'ALL PRODUCT') as product_name,
			discount,
			start_date,
			end_date,
			free_item_id,
			(select name from sma_products where id = free_item_id) as free_item_name,
			min_trx,
			status			
			from sma_product_discounts";
		$this->data["discounts"] = $this->db->query($sql)->result_object();
        $this->page_construct('products/discount', $meta, $this->data);
    }
    ...
    function getProducts($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
            ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image,  {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name,  {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(wp.quantity, 0) as quantity, {$this->db->dbprefix('units')}.code as unit, wp.rack as rack, alert_quantity, '01-01-2020' as expiry", FALSE)
            ->from('products');
            ...
            $this->datatables
                ->select($this->db->dbprefix('products') . ".id as productid, {$this->db->dbprefix('products')}.image as image,  {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name,  {$this->db->dbprefix('categories')}.name as cname, cost as cost, price as price, COALESCE(quantity, 0) as quantity, {$this->db->dbprefix('units')}.code as unit, '' as rack, alert_quantity, '01-01-2020' as expiry", FALSE)
                ->from('products')
	...
	function getDiscounts($warehouse_id = NULL)
    {
        ...
        $detail_link = anchor('admin/products/view_discount/$1', '<i class="fa fa-file-text-o"></i> ' . lang('discount_details'));
        $delete_link = "<a href='#' class='tip po' title='<b>" . $this->lang->line("delete_discount") . "</b>' data-content=\"<p>"
            . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete1' id='a__$1' href='" . admin_url('products/delete_discount/$1') . "'>"
            . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i> "
            . lang('delete_discount') . "</a>";
		...
		<ul class="dropdown-menu pull-right" role="menu">
			<li>' . $detail_link . '</li>
			<li><a href="' . admin_url('products/add_discount/$1') . '"><i class="fa fa-plus-square"></i> ' . lang('duplicate_product') . '</a></li>
			<li><a href="' . admin_url('products/edit_discount/$1') . '"><i class="fa fa-edit"></i> ' . lang('edit_discount') . '</a></li>';
       
       
        $this->load->library('datatables');
        if ($warehouse_id) {
            $this->datatables
            ->select($this->db->dbprefix('discounts') . ".id as productid, {$this->db->dbprefix('products')}.code as code, {$this->db->dbprefix('products')}.name as name,  {$this->db->dbprefix('product_discounts')}.discount as product_discounts, {$this->db->dbprefix('product_discounts')}.start_date as start_date,
			{$this->db->dbprefix('product_discounts')}.end_date as end_date,
			{$this->db->dbprefix('product_discounts')}.min_trx as min_trx,
			wp.name as free_item_name,
			{$this->db->dbprefix('product_discounts')}.status as status
			
			", FALSE)
            ->from('products');
			
			$this->datatables->join("( SELECT * from {$this->db->dbprefix('product_discounts')}) wp", 'products.id=wp.product_id');
			
			$this->datatables->join("( SELECT * from {$this->db->dbprefix('products')}) wp2", 'products.id=wp2.free_item_id', 'right');
			
            if ($this->Settings->display_all_products) {
                $this->datatables->join("( SELECT product_id, quantity, rack from {$this->db->dbprefix('warehouses_products')} WHERE warehouse_id = {$warehouse_id}) wp", 'products.id=wp.product_id');
            } else {
                $this->datatables->join('warehouses_products wp', 'products.id=wp.product_id', 'left')
                ->where('wp.warehouse_id', $warehouse_id)
                ->where('wp.quantity !=', 0);
        ...
        $this->datatables->add_column("Actions", $action, "productid, code, name, product_discounts, start_date, end_date, min_trx, free_item_name, status");
        echo $this->datatables->generate();
    }
    ...
    function add($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->addProduct($data, $items, $warehouse_qty, $product_attributes, $photos)) {
            ...
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Menambah produk dengan kode $product', 'sma_products', '$ip', '$id')");
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->products_model->updateProduct($id, $data, $items, $warehouse_qty, $product_attributes, $photos, $update_variants)) {
            ...
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Memperbarui produk dengan kode $product', 'sma_products', '$ip', '$id')");
    ...
    function update_price()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (isset($_FILES["userfile"])) {
                ...
                $rw = 2;
                foreach ($final as $csv_pr) {
                    $this->products_model->getProductByCode(trim($csv_pr['code']));
	...
	function add_discount()
	{
		...
		$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('discount'), 'page' => lang('discount')), array('link' => '#', 'page' => "Add Discount"));
		$meta = array('page_title' => "Add Discount", 'bc' => $bc);
		$this->page_construct('products/add_discount', $meta, $this->data);
	}
	
	function edit_discount($id)
	{
		$this->data['error'] = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
		$product = $this->db->query("select * from `sma_product_discounts` where id = $id")->row();
		
		$this->data['product'] = $product;
		
		$bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('discount'), 'page' => lang('discount')), array('link' => '#', 'page' => "Edit Discount"));
		$meta = array('page_title' => "Edit Discount", 'bc' => $bc);
		$this->page_construct('products/edit_discount', $meta, $this->data);
	}
...
// 		if($product_id != -1)
// 		 $sql = "INSERT INTO `sma_product_discounts`( `product_id`, `discount`, `start_date`, `end_date`, `free_item_id`, `min_trx`, `status`) select id, '$discount', '$start_date', '$end_date', '$free_item_id', '$min_trx', $status from sma_products where id = $product_id";
// 		 else $sql = "INSERT INTO `sma_product_discounts`( `product_id`, `discount`, `start_date`, `end_date`, `free_item_id`, `min_trx`, `status`) select -1, '$discount', '$start_date', '$end_date', '$free_item_id', '$min_trx', $status  ";

// 	    echo $sql;die;
// 		$this->db->query($sql);
// 		$this->session->set_flashdata('message', 'Discount Saved');
//         admin_redirect('products/discount');
// 	}
...
// 		$sql = "update `sma_product_discounts` set `product_id`=$product_id, `discount`=$discount, `start_date`='$start_date', `end_date`='$end_date', `free_item_id`='$free_item_id', `min_trx`='$min_trx', `status`='$status' where id = $id";
	
		
// 		$this->db->query($sql);
// 		$this->session->set_flashdata('message', 'Discount Updated');
//         admin_redirect('products/discount');
// 	}
	function save_discount()
	{
		...
		$sql = "INSERT INTO `sma_product_discounts`( `product_id`, `discount`, `start_date`, `end_date`, `free_item_id`, `min_trx`, `status`) VALUES ($product_id, '$discount', '$start_date', '$end_date', '$free_item_id', '$min_trx', $status)";
	...
	function update_discount()
	{
		...
		$this->db->update("sma_product_discounts", $data, array("id" => $data["id"]));
    ...
    {
        $query = $this->db->query("SELECT product_id FROM sma_product_discounts WHERE id = '$id'")->result();
		...
		$this->db->query("delete from sma_product_discounts where id = $id");
        if (true) {
            $id = $this->session->userdata('user_id');
            $ip = $_SERVER['REMOTE_ADDR'];
            
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Menghapus discount dengan id $id', 'sma_product_discounts', '$ip', '$id')");

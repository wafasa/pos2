...
//        }
    ...
    function getProductsReport($pdf = NULL, $xls = NULL)
    {
        ...
        $query_return_sales ="( SELECT si.product_id, SUM( si.quantity ) soldQty, SUM( si.subtotal ) totalSale 
                    from " . $this->db->dbprefix('sales') . " s 
                    JOIN " . $this->db->dbprefix('sale_items') . " si on s.id = si.sale_id 
                    WHERE 1
                    AND si.quantity <= 0";
        ...
        if ($start_date || $warehouse) {
            ...
            if ($start_date) {
                ...
                $query_free_item .= " and s.date >= '{$start_date}' AND s.date < '{$end_date}' ";
                $query_return_sales .= " and s.date >= '{$start_date}' AND s.date < '{$end_date}' ";
                if ($warehouse) {
                    $pp .= " AND ";
                    $sp .= " AND ";
                    $query_free_item .= " AND ";
                    $query_return_sales .= " AND ";
                }
            }
            if ($warehouse) {
                $pp .= " pi.warehouse_id = '{$warehouse}' ";
                $sp .= " si.warehouse_id = '{$warehouse}' ";
                $query_free_item .= " si.warehouse_id = '{$warehouse}' ";
                $query_return_sales .= " si.warehouse_id = '{$warehouse}' ";
            }
        }
        $pp .= " GROUP BY pi.product_id ) PCosts";
        $sp .= " GROUP BY si.product_id ) PSales";
        $query_free_item .= " GROUP BY si.product_id ) PFeeItem";
        $query_return_sales .= " GROUP BY si.product_id ) PReturnSales";
...
//             echo'<pre>';print_r($this->datatables->generate());exit();
            echo $this->datatables->generate();
    ...
    function daily_sales($warehouse_id = NULL, $year = NULL, $month = NULL, $pdf = NULL, $user_id = NULL)
    {
        ...
        if (!empty($sales)) {
            foreach ($sales as $sale) {
                $daily_sale[$sale->date] = "<table class='table table-bordered table-hover table-striped table-condensed data' style='margin:0;'><tr><td>" . lang("total") . "</td><td>" . $this->sma->formatMoney($sale->total) . "</td></tr></table>";
            }
        } else {
            $daily_sale = array();
        }
    ...
    // OLD 2017-12-17
    // function getSalesReportDetail($pdf = NULL, $xls = NULL)
    // {
    ...
    //         if ($taxed) {
    //             if( $taxed==1 ){
    //                 $this->db->where('sales.order_tax_id', 1);
    //             }else{
    //                 $this->db->where('sales.order_tax_id !=', 1);
    //             }
    //         }
    ...
    //             $this->excel->getActiveSheet()->SetCellValue('F1', 'Harga Item');
    ...
    //                 $this->excel->getActiveSheet()->SetCellValue('F' . $row, $data_row->unit_price);
    //                 $this->excel->getActiveSheet()->SetCellValue('G' . $row, $data_row->qty);
        ...
        function getSalesReportDetail($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("sales.date, sales.reference_no, biller, customer, payments.paid_by,
                {$this->db->dbprefix('sale_items')}.product_name as product_name,
                {$this->db->dbprefix('sale_items')}.unit_price as unit_price, 
                {$this->db->dbprefix('sale_items')}.unit_quantity as qty, 
                (unit_price * unit_quantity) as grand_total, 
                (unit_price * unit_quantity) AS paid,  
                payment_status", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
...
//            exit();
            ...
            if (!empty($data)) {
                ...
                $this->excel->getActiveSheet()->SetCellValue('H1', lang('grand_total'));
                $this->excel->getActiveSheet()->SetCellValue('I1', lang('paid'));
                $this->excel->getActiveSheet()->SetCellValue('J1', "Paid By");
            ...
            $this->datatables
                ...
                ->select("DATE_FORMAT(sma_sales.date, '%Y-%m-%d %T') as date, 
                    sales.reference_no, 
                    biller, 
                    customer, 
                    FSI.product_code,
                    ...
                    FSI.unit_price, 
                    FSI.qty,
                    (FSI.unit_price * FSI.qty), 
                    (FSI.unit_price * FSI.qty) AS subtotal, 
                    payments.paid_by,
                    (grand_total-paid) as balance,
                    payment_status", FALSE)
                ->from('sales')
...
//            exit();
    ...
    function getSalesReport($pdf = NULL, $xls = NULL)
    {
        ...
        if ($pdf || $xls) {
            ...
            $this->db
                ->select("sma_sales.date, sma_sales.reference_no, biller, customer, GROUP_CONCAT(CONCAT(" . $this->db->dbprefix('sale_items') . ".product_name, ' (', " . $this->db->dbprefix('sale_items') . ".quantity, ')') SEPARATOR '\n') as iname, grand_total, paid, payment_status, payments.paid_by,", FALSE)
                ->from('sales')
                ->join('sale_items', 'sale_items.sale_id=sales.id', 'left')
                ->join('warehouses', 'warehouses.id=sales.warehouse_id', 'left')
                ->join('payments', 'payments.sale_id=sales.id', '')

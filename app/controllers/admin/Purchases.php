...
//generate pdf and force to download
    ...
    public function add($quote_id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->addPurchase($data, $products)) {
            ...
            $ref = $this->input->post('reference_no') ? $this->input->post('reference_no') : $this->site->getReference('po');
            
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Menambah purchase dengan nomor referensi $ref', 'sma_purchases', '$ip', '$id')");
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->purchases_model->updatePurchase($id, $data, $products)) {
            ...
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Memperbarui purchase dengan nomor referensi $ref', 'sma_purchases', '$ip', '$id')");
            $this->session->set_flashdata('message', $this->lang->line("purchase_added"));
            admin_redirect('purchases');
        } else {
    ...
    {
        $query = $this->db->query("SELECT reference_no FROM sma_purchases WHERE id = '$id'")->result();
        ...
        if ($this->purchases_model->deletePurchase($id)) {
            ...
            $this->db->query("INSERT INTO sma_logs (log_act, log_table, log_ip, log_user) VALUES ('Menghapus purchase dengan nomor referensi $ref', 'sma_purchases', '$ip', '$id')");

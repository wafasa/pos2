...
class Sales_model extends CI_Model
{
    ...
    public function getDiscount($product_id = NULL, $total_transaksi = NULL)
    {
        $total_transaksi = empty($total_transaksi) ? 0 : $total_transaksi;
        $sql = "SELECT discount, free_item_id from sma_product_discounts 
                   WHERE current_date 
                       BETWEEN start_date AND end_date 
                       AND (((product_id = $product_id AND $total_transaksi >= min_trx) OR product_id = -1) 
                            OR ($total_transaksi >= min_trx AND min_trx != 0 AND product_id = 0 )) 
                       AND status = 1";
